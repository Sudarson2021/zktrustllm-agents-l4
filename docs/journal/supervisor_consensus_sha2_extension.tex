...
No Raft/QBFT number is inserted until three clean real-cluster sessions pass
the retained-observation, implementation-hash, quorum-loss, recovery, and
manifest-integrity gates.
}
...
\paragraph{Local hash-primitive reference.}
...
algorithm/input cell with cyclically balanced order. The reported
\texttt{gasleft()} deltas are specific to Solidity 0.8.20, optimizer 200,
\texttt{viaIR=true}, and the Paris EVM target. The pure-Solidity SHA-512
implementation is an experimental instrument only; the deployable workflow
computes SHA-512 off chain and anchors its complete 64-byte digest.
...
No local primitive number is inserted unless the clean-source,
cryptographic-vector, retained-payload, configuration, and repetition gates
pass.
}
...
available, either measure anchoring of precomputed complete digests or the
separately labelled local primitive path. They do not measure encryption, hash
security, CPU throughput, consensus superiority, economic finality, or
production O-RAN performance.
