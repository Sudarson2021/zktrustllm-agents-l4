...
## Verified critical follow-up audits (14 August 2026)

- **ZK decision:** the retained 240-row matrix supports 0/240 row-bound
  successful verifications. The selected revision title is *Auditable
  Policy-Governed LLM-Agent Control for O-RAN Edge Security*; system-level
  zero-knowledge wording is removed.
- **Oracle statistics:** the frozen 180 cells were analysed with 20,000
  whole-scenario bootstrap replicates. Decision outputs were repeat-stable in
  18/18 scenario-mode groups and enforced actions in 16/18. Exact per-mode
  estimates and descriptive intervals are in `main_text_dropins.tex` and
  `verified_audit_results_20260814.json`.
- The independently checked archive SHA-256 values are
...
3. Retain the verified external-baseline, open-weights, injection, metadata,
   HTTP, ZK-coverage, and oracle-statistics archives. Do not run or report human
   annotation for this submission.
