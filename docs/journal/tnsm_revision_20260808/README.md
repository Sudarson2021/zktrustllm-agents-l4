...
| 17 | Human oracle validation | **Stage 8 execution required** | Stage 8A creates two separately ordered, oracle-blinded 30-cell packages plus a coordinator-only key. Stage 8B reports decision, action, and joint Cohen’s kappa at cell level and a six-scenario clustered sensitivity analysis. Two independent O-RAN-literate annotators and institutional ethics/data-protection confirmation are required. |
