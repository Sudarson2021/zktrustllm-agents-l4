...
\node[literature,text width=2.1cm,right=3mm of r3] (r4)
  {\IfFileExists{derived_consensus/results_permissioned_faults.tex}
    {crash/quorum loss\\\textbf{measured locally}}
    {crash faults only\\\textbf{reference only}}};
...
\node[extension,text width=2.1cm,right=3mm of q3] (q4)
  {\IfFileExists{derived_consensus/results_permissioned_faults.tex}
    {non-participation\\\textbf{measured locally}}
...
measurement. QBFT is the selected Byzantine-fault-tolerant EVM extension.
\IfFileExists{derived_consensus/results_permissioned_faults.tex}
{The local-cluster extension measures process crash/non-participation, quorum
loss, and recovery for real etcd/Raft and Besu/QBFT deployments; it does not
inject arbitrary Byzantine messages or compare their absolute latencies.}
{No Raft or QBFT latency is reported until real multi-node runs pass the
three-session evidence gate.}}
