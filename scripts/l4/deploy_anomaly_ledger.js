...
async function main() {
  ...
  const AnomalyLedger = await hre.ethers.getContractFactory("AnomalyLedger");
  const ledger = await AnomalyLedger.deploy();
  await ledger.waitForDeployment();

  const ledgerAddress = await ledger.getAddress();
  dep.anomalyLedger = ledgerAddress;
  dep.updatedAt = new Date().toISOString();

  fs.writeFileSync(depFile, JSON.stringify(dep, null, 2));

  console.log("AnomalyLedger :", ledgerAddress);
