...
OUT_FILE = OUT_DIR / "proof_payload.auth_v2_2.json"
...
def main():
    payload = {
        "relationVersion": "auth_v2_2_relation",
    ...
    trace_commitment = (
        ...
        + payload["policyAdmissibilityFlag"]
        + payload["trustState"]
    ...
    payload["notes"] = {
        "purpose": "First AUTH_V2.2 trust-state-aware proof payload",
        "source": "Restricted -> Isolate admissible path"
    }

    with open(OUT_FILE, "w") as f:
        json.dump(payload, f, indent=2)

    print("Saved AUTH_V2.2 proof payload ->", OUT_FILE)
    print("relationVersion ->", payload["relationVersion"])
    print("agentId ->", payload["agentId"])
    print("trustState ->", payload["trustState"], payload["trustStateText"])
    print("actionClass ->", payload["actionClass"], payload["actionClassText"])
