...
payload = json.load(open(ROOT / "runtime_artifacts" / "l4" / "auth_v2_2" / "proof_payload.auth_v2_2.frozen.json"))
proof = json.load(open(ROOT / "runtime_artifacts" / "l4" / "auth_v2_2" / "proof.frozen.json"))
artifact = json.load(open(ROOT / "artifacts" / "contracts" / "l4" / "DecisionAttestorAuthV2_2.sol" / "DecisionAttestorAuthV2_2.json"))
...
print("Verifier ->", dep["authV2_2Groth16Verifier"])
print("Input count ->", len(inputs))
print("Preflight submitDecision() ->", preflight)
print("Submitted AUTH_V2.2 decision tx ->", receipt["transactionHash"].hex())
...
print("Stored trustState ->", stored[8])
